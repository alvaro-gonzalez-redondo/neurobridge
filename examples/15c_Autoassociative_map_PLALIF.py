...
class RelationalNetworkExperiment(PersistentExperiment):
    ...
    alif_params = {
        ...
        'tau_max': 50.0,
        'beta': 1.7e-3,
        'r_target_hz': 50.0 ,      # Hz objetivo (conceptual)
        'eta_theta': 1e-4,    # learning rate intrínseco (muy lento)
    }
    ...
    def build_network(self):
        ...
        with self.sim.autoparent("normal"):
            ...
            is_neuron_subset = is_multiple_of(20)
            is_synapse_subset = is_multiple_of(2000)

            if True:
                self.inp_monitor = SpikeMonitor(
                    [
                        self.inputs["PopA"].where_idx(lambda i: is_neuron_subset(i)),
                        self.inputs["PopB"].where_idx(lambda i: is_neuron_subset(i)),
                        self.inputs["PopC"].where_idx(lambda i: is_neuron_subset(i)),
                        self.inputs["PopH"].where_idx(lambda i: is_neuron_subset(i)),
                    ]
                )

                self.pop_monitor = SpikeMonitor(
                    [
                        self.pops_exc["PopA"].where_idx(lambda i: is_neuron_subset(i)),
                        self.pops_inh["PopA"].where_idx(lambda i: is_neuron_subset(i)),

                        self.pops_exc["PopB"].where_idx(lambda i: is_neuron_subset(i)),
                        self.pops_inh["PopB"].where_idx(lambda i: is_neuron_subset(i)),

                        self.pops_exc["PopC"].where_idx(lambda i: is_neuron_subset(i)),
                        self.pops_inh["PopC"].where_idx(lambda i: is_neuron_subset(i)),

                        self.pops_exc["PopH"].where_idx(lambda i: is_neuron_subset(i)),
                        self.pops_inh["PopH"].where_idx(lambda i: is_neuron_subset(i)),
                    ]
                )
            ...
            if True:
                self.weight_monitor = VariableMonitor(
                    [
                        self.triplet_conns[0].where_idx(is_synapse_subset),
                    ], 
                    ["weight"]
                )
    ...
    def _build_internal_connectivity(self, pop_exc, pop_inh, inp, is_hidden):
        ...
        self.sim.connect(inp, pop_exc, connection_type=StaticSparse, pattern="one-to-one", channel=0, weight=Constant(25.0), delay=UniformInt(0,19))
        ...
        # Exc -> Inh (Estático)
        self.sim.connect(pop_exc, pop_inh, connection_type=StaticSparse, pattern="random", p=self.p_conn, channel=0, weight=Constant(1e-4), delay=UniformInt(0,self.delay_max-1))

        # Inh -> Exc (Plasticidad Inhibitoria Vogels)
        self.sim.connect(pop_inh, pop_exc, connection_type=StaticSparse, pattern="random", p=self.p_conn, autapses=False, channel=1, weight=Constant(1e-0), delay=UniformInt(0,5))

        # Inh -> Inh (Estático)
        #self.sim.connect(pop_inh, pop_inh, connection_type=StaticSparse, pattern="random", p=self.p_conn, autapses=False, channel=1, weight=Constant(3e-5), delay=UniformInt(0,5))
        self.sim.connect(pop_inh, pop_inh, connection_type=StaticSparse, pattern="random", fanin=int(self.p_conn*self.N_exc), autapses=False, channel=1, weight=Constant(1e-4), delay=UniformInt(0,5))
