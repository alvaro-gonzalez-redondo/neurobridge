...
class NeuronTests(Experiment):
    ...
    plalif_params = {
        ...
        'tau_max': 50.0,
        'beta': 1.7e-3,
        'r_target_hz': 5.0 ,      # Hz objetivo (conceptual)
    ...
    def build_network(self):
        ...
        with self.sim.autoparent("normal"):
            ...
            self.in_spikes = RealtimeSpikeMonitor(
                #groups = [self.inputs_ampa.where_idx(lambda i: i<50), self.inputs_gaba.where_idx(lambda i: i<50)], group_names = ["AMPA", "GABA"],
                groups = [self.inputs_ampa.where_idx(lambda i: i<50)], group_names = ["AMPA"],
                viz_client=self.viz, 
                plot_id="raster_1",
            ...
            self.out_spikes = RealtimeSpikeMonitor(
                #groups = [self.output1, self.output2, self.output3], group_names = ["LIF", "ALIF", "PowerLawALIF"],
                groups = [self.output3], group_names = ["PowerLawALIF"],
                viz_client=self.viz, 
                plot_id="raster_2",
                rollover_spikes=1_000,
                rollover_lines=500,
            ...
            _ = RealtimeVariableMonitor(
                [self.output3], 
                ['threshold_base'],
                viz_client=self.viz, plot_id="variable_3",
                interval=100, rollover=100
            )
    ...
    def pre_step(self):
        ...
        t = torch.tensor(step*1e-3, device=self.current_device, dtype=torch.float32)
        t1 = t*11 / 50  
        t2 = t*13 / 50
