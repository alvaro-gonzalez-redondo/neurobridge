...
class SFAToyProblem(Experiment):
    def build_network(self):
        ...
        with self.sim.autoparent("normal"):
            ...
            self.output = ALIFNeurons(1, beta=1e-5, tau_adapt=300.0)
            self.output.A[:] = 3000.0 #No tengo paciencia para esperar que las neuronas se adapten en 5 min
            ...
            self.input2output = self.sim.connect(
                ...
                pattern="all-to-all",
                weight=Constant(1e-2),
                tau_stdp_fast=50e-3,
                tau_stdp_slow=100e-3,
                scale=1e-2,
                norm_every=1,
                A=-5e-6, #Learning rate
                channel=0,
            )
       
        with self.sim.autoparent("normal"):
            self.spike_monitor = SpikeMonitor([self.inputs.where_idx(lambda i: i==j) for j in range(self.n_inputs)] + [self.output])
            self.weight_monitor = VariableMonitor([self.input2output], ["weight"])
    ...
    def on_finish(self):
        ...
        ax1 = ax0.twinx()
        id_sum = 0
        labels = [f"x{i}" for i in range(self.n_inputs)] + ["y"]
        for idx, label in enumerate(labels):
            ...
            ax1.scatter(spk_times, (spk_neurons*0)+id_sum, s=40, alpha=0.5, label=label, c=f"C{idx}" if label!="y" else "black")
            ...
            if label == "y":
                ...
                ax0.plot(times, rate, label=label, c="black", linewidth=5)
            else:
                times = np.arange(0,self.current_time, self.sim.dt)
                rep_times = int(np.ceil(self.current_step / self.input_rates.shape[1]))
                signal = np.tile(self.input_rates[:,idx], rep_times)[:self.current_step]
                ax0.plot(times, signal, label=label, c=f"C{idx}", linewidth=1)
        ...
        from_time = 0.0
        times = np.linspace(from_time, to_time, num=w_values.shape[0])
        labels = [f"x{i}" for i in range(self.n_inputs)]

        for idx, label in enumerate(labels):
            ax0.plot(times, w_values[:, idx], label=label, c=f"C{idx}", linewidth=3)
